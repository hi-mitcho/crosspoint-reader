...
// Longest measurable/drawable span. Wrapped lines stay under the screen
// width (far below this); only pathological unbreakable tokens are split at
// this cap. Matches DictionaryDefinitionActivity's cap.
constexpr size_t MAX_LINE_BYTES = 191;

// Body text left/right inset, matching the reader's default feel.
constexpr int SIDE_PADDING = 20;

// htmlToPlainText() allocates an output string roughly the size of its
// input (reserve(html.size())) plus wrapText()'s own line-span vector and SD
// font prewarm scratch. With -fno-exceptions a failed allocation aborts the
// device (see firmware/CLAUDE.md's `new` rule) rather than throwing, so this
// must be checked against the article's actual size before attempting it —
// a fixed threshold (as DictHtmlPages.cpp uses for its own, size-independent
// styled-path buffers) isn't enough here since the required block scales
// with the article.
constexpr size_t PLAIN_TEXT_HEAP_MARGIN = 8 * 1024;

}  // namespace

void ArticleReaderActivity::onEnter() {
  Activity::onEnter();
  if (!layoutHtmlPages()) {
    const size_t required = html.size() + PLAIN_TEXT_HEAP_MARGIN;
    if (ESP.getMaxAllocHeap() < required) {
      LOG_ERR("AREAD", "Not enough contiguous heap for plain-text fallback (%zu needed, %u max block)", required,
              ESP.getMaxAllocHeap());
      loadFailed = true;
      html.clear();
      html.shrink_to_fit();
...
void ArticleReaderActivity::onExit() {
  Activity::onExit();
...
ArticleReaderActivity::BodyArea ArticleReaderActivity::bodyArea() const {
...
// Styled path: lay the article HTML out through the EPUB chapter parser into
// reader-identical Pages. buildDictionaryHtmlPages() self-gates on heap and
// page/element counts, so a huge or malformed article simply returns false
// here rather than risking OOM; the caller falls back to plain text. Frees
// `html` on success (the page arenas own the text).
bool ArticleReaderActivity::layoutHtmlPages() {
  const BodyArea body = bodyArea();
  if (body.width <= 0 || body.height <= 0) return false;
  if (!buildDictionaryHtmlPages(renderer, html, static_cast<uint16_t>(body.width), static_cast<uint16_t>(body.height),
                                pages)) {
    return false;
  }
  html.clear();
...
int ArticleReaderActivity::measureSpan(const int fontId, const char* text, size_t len) const {
...
// Greedy word-wrap of `html` (already plain text at this point) into byte
// spans. '\n' breaks lines; blank lines survive as paragraph spacing.
void ArticleReaderActivity::wrapText() {
  lines.clear();
  lines.reserve(html.size() / 32 + 8);

  const int fontId = SETTINGS.getReaderFontId();
  renderer.ensureSdCardFontReady(fontId, html.c_str(), 0x01 /* REGULAR */);
  ...
  const uint32_t n = static_cast<uint32_t>(html.size());
  ...
  while (i < n) {
    ...
    // complete codepoint so measure/draw never see a partial sequence.
...
void ArticleReaderActivity::loop() {
...
// for the real paint.
void ArticleReaderActivity::drawBody(const int fontId, const int x, const int startY) const {
  if (!pages.empty()) {
  ...
  for (int i = firstLine; i < lastLine; i++) {
    ...
    memcpy(buf, html.c_str() + lines[i].start, len);
    buf[len] = '\0';
    renderer.drawText(fontId, x, startY + (i - firstLine) * lineHeight, buf);
  }
}

void ArticleReaderActivity::render(RenderLock&&) {
  ...
  // Header: article title left (truncated to one line with an ellipsis if
  // it doesn't fit), page counter right.
  ...
  const int titleGap = counterWidth > 0 ? 12 : 0;
  const int titleMaxWidth = contentWidth - 2 * SIDE_PADDING - counterWidth - titleGap;
  const auto titleLines =
      renderer.wrappedText(UI_12_FONT_ID, title.c_str(), std::max(0, titleMaxWidth), 1, EpdFontFamily::BOLD);
  if (!titleLines.empty()) {
    renderer.drawText(UI_12_FONT_ID, contentX + SIDE_PADDING, headerY, titleLines[0].c_str(), true,
                      EpdFontFamily::BOLD);
  }
  if (counterWidth > 0) {
  ...
  if (loadFailed) {
    renderer.drawText(UI_10_FONT_ID, contentX + SIDE_PADDING, bodyStartY, tr(STR_ARTICLE_READ_FAILED));
  } else {
    // Body: two-pass draw inside a prewarm scope (same pattern as the
    // reader's renderContents) so SD-card font glyphs load from SD in one
    // batch instead of one on-demand overflow read per character on every
    // page turn.
    const int fontId = SETTINGS.getReaderFontId();
